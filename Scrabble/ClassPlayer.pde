...
class Player {
  ...
  Player(PApplet p, int i) {
    ...
    revert = new Button(p, P_XOFF + 180, index*P_H + 110, 180, 30, "Change move") {//change the text if you want
      @Override
        void pressed() {
        revert();
      }
      ...
      if(cells.length > 1){
        ...
        for (int n = 0; n < cells.length; n++) {
          ...
          if (!ok) {
            int i = s.i;
            int j = s.j;
            if (grid[i+1][j].used || grid[i-1][j].used || grid[i][j+1].used || grid[i][j-1].used)ok = true;
          }
          Word next = null;
          try {
            if (v) {
