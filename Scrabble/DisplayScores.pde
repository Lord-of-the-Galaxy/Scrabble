  ...
  if(words.size() <= 5){  
    ...
    for(int i = 0; i < words.size(); i++){
      ...
      if(mouse && mouseY > ST_Y + 35 + h*i && mouseY < ST_Y + 35 + h*(i+1)){
        ...
        noStroke();
        fill(GREEN);
        if(i != 4)rect(P_XOFF + 2, ST_Y + 37 + h*i, ST_W - 4, h - 2);
        else rect(P_XOFF + 2, ST_Y + 37 + h*i, ST_W - 4, h - 2, 0, 0, 6, 6);
...
void displayWordScoreBreakup(Word w){
  ...
  for(int i = 0; i < w.val.length(); i++){
    if(i != w.val.length()-1 && 35 + h*(i+1) - W_S_H < 155)line(W_S_X, W_S_Y + 35 + h*(i+1), W_S_X + W_S_W, W_S_Y + 35 + h*(i+1));
